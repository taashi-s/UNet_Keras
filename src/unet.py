...
class UNet(object):
    def __init__(self, input_shape, class_num, filters_list=None, trainable_list=None):
        ...
        if trainable_list is None:
            trainable_list = [False, False, False, True, True]
        layer = inputs
        encodeLayers = []
        for k, (filters, trainable) in enumerate(zip(filters_list, trainable_list)):
            layer = self.__add_encode_layers(filters, layer, is_first=(k==0), trainable=trainable)
            encodeLayers.append(layer)

        add_drop_layer_indexes = [1, 2, 3]
        deconv_item = zip(reversed(filters_list[:-1]), reversed(encodeLayers[:-1]), reversed(trainable_list[:-1]))
        for k, (filters, concat_layer, trainable) in enumerate(deconv_item):
    ...
    def __add_encode_layers(self, filter_size, input_layer, is_first=False, trainable=True):
        layer = input_layer
        if is_first:
            layer = Conv2D(filter_size, 3, padding='same', input_shape=self.INPUT_SHAPE, trainable=trainable)(layer)
        else:
            layer = MaxPooling2D(2)(layer)
            layer = Conv2D(filter_size, 3, padding='same', trainable=trainable)(layer)
    ...
    def __add_decode_layers(self, filter_size, input_layer, concat_layer, add_drop_layer=False, trainable=True):
